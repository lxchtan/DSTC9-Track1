...
def write_generation_preds_debug(dataset_walker, output_file, dialog_ids, responses, anothers):
    labels = [label for log, label in dataset_walker]
    new_labels = [{"target": False}] * len(dataset_walker)
    # Update the dialogs with detection result
    for dialog_id, response, another in zip(dialog_ids, responses, anothers):
        label = labels[dialog_id]
        new_label = {"target": True, "response": response}
        new_label.update(another)
        if label is None:
            label = new_label
        else:
            label = label.copy()
...
def pad_input_masks(arrays, padding, max_length=-1):
    if max_length < 0:
        max_length = max(list(map(len, arrays)))

    arrays = [
        [
            row + [padding] * (max_length - len(row))
            for row in array
        ]
        for array in arrays
    ]
