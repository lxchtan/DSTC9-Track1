...
from .utils.training import get_classes, regain_loss
...
from .utils.auxiliary import SequentialDistributedSampler
...
def train(args, train_dataset, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, run_batch_fn_train,
  ...
  for train_epoch in train_iterator:
    train_one_epoch(args, model, optimizer, run_batch_fn_train, scheduler,
                    train_dataloader, train_epoch, t_total, tb_writer)
    train_eval_one_epoch(args, eval_dataset, model, run_batch_fn_eval, tb_writer, tokenizer)

  if args.local_rank in [-1, 0]:
    tb_writer.close()


def train_eval_one_epoch(args, eval_dataset, model, run_batch_fn_eval, tb_writer, tokenizer):
  global global_step
...
def train_one_epoch(args, model, optimizer, run_batch_fn_train, scheduler,
                    train_dataloader, train_epoch, t_total, tb_writer):
  global global_step
  local_steps = 0
  tr_loss = 0.0
  multi_loss_sum = np.zeros(1)
  epoch_iterator = tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])
  for step, batch in enumerate(epoch_iterator):
    model.train()
    loss, _, loss_2, loss_3 = run_batch_fn_train(args, model, batch)

    loss, multi_loss = regain_loss(args, loss, loss_2, loss_3, train_epoch, percentage=global_step / t_total)
    ...
    if args.task == 'generation':
      if args.n_gpu > 1:
        multi_loss = multi_loss.mean(axis=-1)
      if args.gradient_accumulation_steps > 1:
        multi_loss /= args.gradient_accumulation_steps
      multi_loss_sum = multi_loss_sum + multi_loss
    ...
    if (step + 1) % args.gradient_accumulation_steps == 0:
      ...
      if args.task == 'generation':
        display_infos.update(zip(['lm', 'bow', 'norm', 'kld'], multi_loss_sum / local_steps))
        write_infos.extend(zip(map(lambda x: f'loss/{x}',
                                   ['lm_loss', 'bow_loss', 'norm_loss', 'kld_loss']),
                               multi_loss_sum / local_steps))
      epoch_iterator.set_postfix(display_infos)
      tb_writer.add_scalar("lr", scheduler.get_lr()[0], global_step)
      any(map(tb_writer.add_scalar, *zip(*map(
        lambda x, y: (*x, y), write_infos, repeat(global_step)
      ))))

  return global_step, local_steps, tr_loss, multi_loss_sum


def evaluate(args, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer,
             run_batch_fn, desc="", use_distributed_sampler=False) -> Dict:
  if args.local_rank in [-1, 0]:
  ...
  eval_sampler = SequentialSampler(eval_dataset) \
    if args.local_rank == -1 or not use_distributed_sampler \
    else SequentialDistributedSampler(eval_dataset)
  ...
  if args.task.lower() == "generation":
    ...
    if args.output_file:
      ...
      if args.local_rank == -1:
        ...
        pickle.dump((data_infos, sorted_pred_ids),
                    open(os.path.join(output_dir, "gpu{}_ks_output.pkl".format(args.local_rank)), 'wb'))
        torch.distributed.barrier()
        if args.local_rank == 0:
          data_infos, sorted_pred_ids = [], []
          for i in range(torch.distributed.get_world_size()):
            di, spi = pickle.load(open(os.path.join(output_dir, "gpu{}_ks_output.pkl".format(i)), 'rb'))
            data_infos.extend(di)
            sorted_pred_ids.extend(spi)
...
def main():
  ...
  # Please don't edit next line to change the default value "0".
  parser.add_argument("--z_hidden_size", type=int, default=5,
                      help="z_hidden_size for plato")
  ...
  if args.eval_only:
    ...
    # set some params for config
    if args.task == 'generation':
      config.dot_token = tokenizer.convert_tokens_to_ids(".")
      config.z_hidden_size = args.z_hidden_size
  ...
  if not args.eval_only:
    ...
    train(args, train_dataset, eval_dataset, model, tokenizer, run_batch_fn_train, run_batch_fn_eval)
    ...
    eval_dataset = dataset_class(dataset_args, tokenizer, split_type=args.eval_dataset, labels=not args.no_labels,
                                 labels_file=args.labels_file)
    result = evaluate(args, eval_dataset, model, tokenizer, run_batch_fn_eval,
                      desc=args.eval_desc or "val", use_distributed_sampler=True)
