...
python3 -m torch.distributed.launch --nproc_per_node ${num_gpus} baseline_modify.py \
        --params_file baseline_modify/configs/detection/params.json \
        --dataroot data \
...
python3 -m torch.distributed.launch --nproc_per_node ${num_gpus} baseline_modify.py \
    ...
    --params_file baseline_modify/configs/selection/params.json \
    --dataroot data \
    --exp_name ks-all-${version}

# Response generation
python3 -m torch.distributed.launch --nproc_per_node ${num_gpus} baseline_modify.py \
    --params_file baseline_modify/configs/generation/params.json \
    --dataroot data \
